...
export function downloadSessionBinary(
  ...
  const binaryBuffer = new Uint8Array(encoded).buffer;
  downloadBlob(
    binaryBuffer,
    safeDownloadFilename(filename, `motion-session${SESSION_BINARY_EXTENSION}`),
    "application/vnd.ardy.session",
  );
}

export function downloadMotionJson(
  motion: StructuredMotionResult,
  filename = "motion.json",
): void {
  downloadBlob(
    encodeMotionJson(motion, true),
    safeDownloadFilename(filename, "motion.json"),
    "application/json;charset=utf-8",
  );
}

export function downloadMotionCsv(
  motion: StructuredMotionResult,
  filename = "motion.csv",
): void {
  downloadBlob(
    encodeMotionCsv(motion),
    safeDownloadFilename(filename, "motion.csv"),
